...
def update_summary(pathname):
    if pathname == '/sub_page3a':
        ...
        fig1 = px.line(fires_per_year, x='Year', y='counts', markers=True, title='Number of Fire Detections per Year (2014-2024)')
        ...
        fig5 = px.line(time_series, x='ACQ_DATE', y='counts', markers=True, title='Time Series Analysis of Fire Occurrences (2014-2024)')
        ...
        fig6 = go.Figure()
        fig6.add_trace(go.Scatter(x=monthly_counts.index, y=monthly_counts, mode='lines+markers', name='Monthly Fire Occurrences'))
        fig6.add_trace(go.Scatter(x=monthly_counts.index, y=rolling_avg, mode='lines', name='6-Month Rolling Average', line=dict(color='orange')))
        fig6.add_trace(go.Scatter(x=monthly_counts.index, y=trend, mode='lines', name='Trend Line (Linear Regression)', line=dict(color='red')))
        fig6.update_layout(title='Trend Analysis of Fire Occurrences (2014-2024)', xaxis_title='Date', yaxis_title='Number of Fires')
